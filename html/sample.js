...
app.controller('network', function($scope, $http, $routeParams) {
	...
	$http.get(api + "/network/" + id).then(function (httpResult) {
		...
		$scope.network.firsttime = Math.min.apply(null, $scope.network.connectiontimes);
		$scope.network.lasttime  = Math.max.apply(null, $scope.network.connectiontimes);
		
		var first = roundDate($scope.network.firsttime);
		var last  = roundDate($scope.network.lasttime);
		var now   = time();
		var data  = new Array((roundDate(now) - first) + 1).fill(0);
		
		for (var i = 0; i < $scope.network.connectiontimes.length; i++)
		{
		...
		data.reverse();
		$scope.timechart_data    = [data];
		$scope.timechart_labels  = data.map(function(v,i,a) {
