...
class Grammar(object):
    ...
    def _read_arg_quot(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache['arg_quot'].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        index1 = self._offset
        ...
        if address0 is FAILURE:
            ...
            if address5 is not FAILURE:
                ...
                while address7 is not FAILURE:
                    ...
                    if self._offset < self._input_size:
                        chunk4 = self._input[self._offset:self._offset + 1]
                    if chunk4 is not None and Grammar.REGEX_2.search(chunk4):
                        address7 = TreeNode(self._input[self._offset:self._offset + 1], self._offset)
                        self._offset = self._offset + 1
                    else:
                        address7 = FAILURE
            ...
            if elements2 is None:
                ...
                address0 = self._actions.make_arg_quot(self._input, index4, self._offset, elements2)
                self._offset = self._offset
            if address0 is FAILURE:
                self._offset = index1
        self._cache['arg_quot'][index0] = (address0, self._offset)
        return address0
